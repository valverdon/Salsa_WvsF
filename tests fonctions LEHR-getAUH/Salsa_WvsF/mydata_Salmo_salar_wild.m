...
fileID = fopen('temp2019mat.txt','r');
formatSpec = '%d %f';
sizeA = [2 Inf];
A = fscanf(fileID,formatSpec,sizeA);
fclose(fileID);
tTireland=A';
tTireland(:,2)=repmat(C2K(mean(tTireland(:,2))),size(tTireland,1),1) ;

data.abW = 148;    units.abW = 'd';    label.abW = 'age at birth';             bibkey.abW = 'JoshkaUnpub';     
temp.abW = tTireland(1:148,:) ; units.temp.abW = 'K'; label.temp.abW = 'temperature vs time';
comment.abW = 'Ireland dataset, 0=fertilisation';%f =1
data.abWtest = 148;    units.abWtest = 'd';    label.abWtest = 'age at birth';             bibkey.abWtest = 'JoshkaUnpub';     
temp.abWtest = tTireland(1:148,:) ; units.temp.abWtest = 'K'; label.temp.abWtest = 'temperature vs time';
comment.abWtest = 'Ireland dataset, 0=fertilisation';%f =1
...
%   comment.tp_ireb = 'Mean of Imsa river (Norway); temperature: rough guess - highly variable temperature throughout life(2+1.04)*365.25';
 data.tsp_ireb = (2+1.035)*325.25;    units.tsp_ireb = 'd';    label.tsp_ireb = 'time since birth at spawning';  bibkey.tsp_ireb = 'Hindar2006'; 
  temp.tsp_ireb = C2K(9);  units.temp.tsp_ireb = 'K'; label.temp.tsp_ireb = 'temperature'; 
  comment.tsp_ireb = 'Mean of Burrishoole river. temperature from Joshka data';
data.tsp_ire =(3.86-1)*365.25; % 
  units.tsp_ire = 'd';    label.tsp_ire = 'time since birth at spawning';  bibkey.tsp_ire = 'Hutchings1998'; 
  temp.tsp_ire = C2K(9);  units.temp.tsp_ire = 'K'; label.temp.tsp_ire = 'temperature'; 
  comment.tsp_ire = 'Mean of Irish populations; temperature: rough guess - highly variable temperature throughout life ((river age+sea age+1(fertilization to birth as in Hutchings 1998))-1(we remove it)) , Table2';
...
% data.asp_norI =(1.68+1.22-1)*365.25; % 694;  
%   units.asp_norI = 'd';    label.asp_norI = 'time since birth at puberty';  bibkey.asp_norI = 'Hindar2006'; 
%   temp.asp_norI = C2K(10);  units.temp.asp_norI = 'K'; label.temp.asp_norI = 'temperature'; 
%   comment.asp_norI = 'Mean of Imsa river (Norway); temperature: rough guess - highly variable temperature throughout life, Table3';
...
% data.Lp_fra  = 63.5 ;   units.Lp_spa  = 'cm';  label.Lp_spa  = 'total length at puberty (grilse)'; bibkey.Lp_spa  = 'Hutchings1998';   
%   comment.Lp_spa = 'mean of French population';
data.Lsp_ire  = 61.8 ;   units.Lsp_ire  = 'cm';  label.Lsp_ire  = 'total length at spawning'; bibkey.Lsp_ire  = 'Hutchings1998';   
  comment.Lsp_ire = 'mean of Irish population';
  temp.Lsp_ire= mean(tTireland(:,2));  units.temp.Lsp_ire = 'K'; label.temp.Lsp_ire = 'temperature'; 
% data.Lp_gbr  = 62 ;   units.Lp_gbr  = 'cm';  label.Lp_gbr  = 'total length at puberty (grilse)'; bibkey.Lp_gbr  = 'Hutchings1998';   
%   comment.Lp_gbr = 'mean of Brittish population';
% data.Lp_nor  = 59.5 ;   units.Lp_nor  = 'cm';  label.Lp_nor  = 'total length at puberty (grilse)'; bibkey.Lp_nor  = 'Hutchings1998';   
%   comment.Lp_nor = 'mean of Norwegian population';
...
data.V0 = 4/3*pi*((0.66/2)^3);  units.V0 = 'cm^3'; label.V0 = 'egg volume'; bibkey.V0 = 'JoshkaUnpub';  comment.V0_norM2011 = 'data Ireland';%T measured, f=1
...
%% Max weight %
%%%%%%%%%%%%%%%%%%%%%%%
data.Wwi = 5200  ;   units.Wwi = 'g';   label.Wwi = 'ultimate wet weight';     bibkey.Wwi = 'Berg2001'; comment.Wwi = 'mean wet weight of 6 spawning females,parents from river StjÃ¸rdalselva (Norway), table 1';
...
data.Rnbe_nor  = 9576;   units.Rnbe_nor  = '#/d'; label.Rnbe_nor  = 'maximum reprod rate';     bibkey.Rnbe_nor  = 'Fleming1996';   
  temp.Rnbe_nor = C2K(10);    units.temp.Rnbe_nor = 'K'; label.temp.Rnbe_nor = 'temperature';
  comment.Rnbe_nor = 'maximum female length at spawning at River Imsa (Norway); temperature: rough approximation, highly variable temperature throughout life, table 3';
...
fileID = fopen('matlenW.txt','r');
formatSpec = '%d %f';
sizeLen = [2 Inf];
Len = fscanf(fileID,formatSpec,sizeLen);
fclose(fileID);
Len=Len';
Len(:,2)= Len(:,2)/10;

 %time (d), fork length (cm)
data.tL = Len ;
units.tL = {'d', 'cm'};  label.tL = {'time', 'fork length'};  
temp.tL = tTireland;  units.temp.tL = 'K'; label.temp.tL = 'temperature';
bibkey.tL = 'JoshkaUnpub'; 
comment.tL= 'Length in cm, f=1';
...
fileID = fopen('matweiW.txt','r');
formatSpec = '%d %f';
sizeWei = [2 Inf];
Wei = fscanf(fileID,formatSpec,sizeWei);
fclose(fileID);
Wei=Wei';
data.tWw = Wei ; % time (d), wet weight (g) during starvation
temp.tWw = tTireland;  units.temp.tWw = 'K'; label.temp.tWw = 'temperature'; 
units.tWw = {'d', 'g'};  label.tWw = {'time since fertilisation', 'wet weight'};  
bibkey.tWw = 'JoshkaUnpub'; comment.tWw = 'Data Ireland';%t=ambient (measured) f=1
...
weights.abW = weights.abW;
weights.abWtest = weights.abWtest*0;
weights.tsp_ire = weights.tsp_ire/2;
weights.tsp_ireb = weights.tsp_ireb/2;
...
weights.tL = weights.tL*4;
weights.tWw = weights.tWw*4;
...
metaData.biblist.(bibkey) = ['''@', type, '{', bibkey, ', ' bib, '}'';'];
%
bibkey = 'JoshkaUnpub'; type = 'Article'; bib = [ ... 
'author = {Joshka},' ...
'title = {-},' ...
'journal = {-},' ...
'volume = {-},' ...
