...
data.abF = 148;    units.abF = 'd';    label.abF = 'age at birth';             bibkey.abF = 'JoshkaUnpub';     
temp.abF= tTireland(1:148,:) ; units.temp.abF = 'K'; label.temp.abF = 'temperature vs time';
comment.abF = 'Ireland dataset, 0=fertilisation';%f =1

data.abFtest = 148;    units.abFtest = 'd';    label.abFtest = 'age at birth';             bibkey.abFtest = 'JoshkaUnpub';     
temp.abFtest= tTireland(1:148,:) ; units.temp.abFtest = 'K'; label.temp.abFtest = 'temperature vs time';
comment.abFtest = 'Ireland dataset, 0=fertilisation';%f =1
...
%  data.tb = 148;    units.tb;    label.tb_12_AqG = 'time between hatching and first feeding';             bibkey.tb_12_AqG = 'Handeland2008';     
%   comment.tb_12_AqG = 'AquaGen strain (Norway), calculated from age at birth since hatching in degree-days, f=1';
%   temp.tb_12_AqG = C2K(12);  units.temp.tb_12_AqG = 'K'; label.temp.tb_12_AqG = 'temperature';
...
data.asp_norire = (2+1.93)*365.25;   %=1070
units.asp_norire = 'd';    label.asp_norire = 'time since birth at 1st spawning';             bibkey.asp_norire = 'Hindar2006';     
comment.asp_norire = 'farmed strain (time since birth) ; table3';
temp.asp_norire = C2K(9);  units.temp.asp_norire = 'K'; label.temp.asp_norire = 'temperature'; label.temp.comment.asp_norire = 'T° : rough guess - highly variable temperature throughout life(2+1.93-1)*365.25';%natural T°, natural f.
% temp is mean of Joshka data (same place)
...
data.V0 = 4/3*pi*((0.64/2)^3);  %=0.111
units.V0 = 'cm^3'; label.V0 = 'egg volume'; bibkey.V0 = 'JoshkaUnpub';  comment.V0_norM2011 = 'data Ireland';%T measured, f=1
...
%   temp.Ri = C2K(6);  units.temp.Ri = 'K'; label.temp.Ri = 'temperature'; label.temp.comment.Ri = 'T = rough guess';%f adlib
data.Rnbe=479/0.3; units.Rnbe = '-/d';    label.Rnbe = 'maximum number of eggs';             bibkey.Rnbe = 'Fleming1996b';
  comment.Rnbe = 'Sunndalsora strain, number of live embryos/survival';
  temp.Rnbe = C2K(6);  units.temp.Rnbe = 'K'; label.temp.Rnbe = 'temperature'; label.temp.comment.Rnbe = 'T = rough guess';%f adlib
...
% weights.ab_norM2 = weights.ab_norM2/2;
 weights.abF = weights.abF;%not sure
  weights.abFtest = weights.abFtest*0;%not sure
...
% weights.asp_norNBP=weights.asp_norNBP;
weights.asp_norire=weights.asp_norire;
% weights.ap_norire=weights.ap_norire/2; % 2 age at puberty zero-V data
weights.Lb_NorM2011 = weights.Lb_NorM2011/10; % 4 Length at birth 0-V data.
weights.Lb_NorM2012 = weights.Lb_NorM2012/10;
weights.Lb_NorM2013 = weights.Lb_NorM2013/10;
weights.Lb_NorS2013 = weights.Lb_NorS2013/10;
...
weights.tL = weights.tL*3;%more weigth to Joshka data
weights.tWw = weights.tWw*3;%more weigth to Joshka data
