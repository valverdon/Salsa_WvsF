...
TC_tsp_ire = tempcorr(temp.tsp_ire, T_ref, T_A);
TC_tsp_ireb = tempcorr(temp.tsp_ireb, T_ref, T_A);
...
TC_Lsp_Fleming1996 = tempcorr(C2K(8.59), T_ref, T_A);
TC_Lsp_ire = tempcorr(temp.Lsp_ire, T_ref, T_A);
...
s_M  = L_j/ L_b; % -, acceleration factor for f
  [t, LEHR] = ode45(@ode_LEHR, linspace(0, t0, 1000), InitCond,[], par, cPar, f, s_M, temp.abW(:,1), temp.abW(:,2));%from fecondation to start of experiment
%   Eh(1)=0;
%   for i=2:size(LEHR,1)
...
% tT_tsp_ire = (tau_p_nat - tau_b_nat) / k_M/ TC_tsp_ire;
% tT_tsp_ireb = (tau_p_nat - tau_b_nat) / k_M/ TC_tsp_ireb;
...
%tests seasonality
ap_Ri= tau_p_nat / k_M / TC_Ri_nor; % prediction of age at puberty for Ri data envir. condtions
...
% asp_norI= (tau_p_nat - tau_b_nat) / k_M/ TC_asp_norI + treprod;
 tT_tsp_ire = (tau_p_nat - tau_b_nat) / k_M/ TC_tsp_ire + treprod;
tT_tsp_ireb = (tau_p_nat - tau_b_nat) / k_M/ TC_tsp_ireb + treprod;

%Length-Weight at first reprod
% rT_B_Berg2001 = TC_Lsp_Berg2001 * rho_B * k_M;
rT_B_Fleming1996 = TC_Lsp_Fleming1996 * rho_B * k_M;
rT_B_ire = TC_Lsp_ire * rho_B * k_M;
Lsp_struct_pred_ire = L_i - (L_i - L_p) * exp( - rT_B_ire * treprod);
% Lsp_struct_pred_Berg2001 = L_i - (L_i - L_p) * exp( - rT_B_Berg2001 * treprod);
% Wwsp_pred_ire = Lsp_struct_pred_ire^3 * (1 + f * ome);       % g, wet weight at 1st reprod at f and T
Lsp_pred_ire = Lsp_struct_pred_ire / del_M; % cm, expected physical length at 1st reprod at f and T

% Wwsp_pred_Berg2001 = Lsp_struct_pred_Berg2001^3 * (1 + f * ome);       % g, wet weight at 1st reprod at f and T
% Lsp_pred_Berg2001 = Lsp_struct_pred_Berg2001 / del_M; % cm, expected physical length at 1st reprod at f and T


Lsp_struct_pred_Fleming1996 = L_i - (L_i - L_p) * exp( - rT_B_Fleming1996 * treprod);
Wwsp_pred_Fleming1996 = Lsp_struct_pred_Fleming1996^3 * (1 + f * ome);       % g, wet weight at 1st reprod at f and T
% Lsp_pred_Fleming1996 = Lsp_struct_pred_Fleming1996 / del_M; % cm, expected physical length at 1st reprod at f and T
...
prdData.tsp_ire = tsp_ire;
prdData.tsp_ireb = tsp_ireb;
  prdData.am = aT_m_nat;
...
%   prdData.Ls = Lw_s;
%   prdData.Lsp_norS = Lsp_pred_Berg2001;
%   prdData.Lp_sco = Lw_p_nat /SNFtoTOT;
%   prdData.Lsp_norI  = Lsp_pred_Fleming1996 /SNFtoTOT;
%   prdData.Lp_spaNM = Lw_p_nat /SNFtoTOT;
%   prdData.Lp_canM = Lw_p_nat /SNFtoTOT;
%   prdData.Lp_canR = Lw_p_nat /SNFtoTOT;
%   prdData.Lsp_norSf = Lsp_pred_Fleming1996 / POHtoTOT;
%   prdData.Lsp_norSm = Lsp_pred_Fleming1996 / POHtoTOT;
 prdData.Lsp_ire = Lsp_pred_ire /SNFtoTOT;
...
%   prdData.WwsI = Ww_s;
%   prdData.WwsS = Ww_s;
 prdData.Wwsp_norf = Wwsp_pred_Fleming1996;
 prdData.Wwsp_norm = Wwsp_pred_Fleming1996;
...
% EWw_Nor = (LWw_Nor(:,1) * del_M).^3 * (1 + f_nat * ome); % g, wet weight
  ...
  L_b = l_b * L_m; L_j = l_j * L_m; L_i = l_i * L_m;
  L_0 = 1e-10;  %L0 naissance et non pas début experience (Lb)
