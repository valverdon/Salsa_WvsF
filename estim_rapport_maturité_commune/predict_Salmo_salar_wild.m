...
TC_tsp_nor = tempcorr(temp.tsp_nor, T_ref, T_A);
...
TC_tsp_norI = tempcorr(temp.tsp_norI, T_ref, T_A);
...
TC_LWw_Berg2001 = tempcorr(C2K(10), T_ref, T_A); %no idea of T°
TC_LWw_Fleming1996 = tempcorr(temp.Lsp_norSf, T_ref, T_A); 
...
%   tT_p_ice = (tau_p_nat - tau_b_nat) / k_M/ TC_tp_ice;   % d, time since birth at puberty at f and T
  tT_tp_nor = (tau_p_nat - tau_b_nat) / k_M/ TC_tsp_nor;   % d, time since birth at puberty at f and T
...
%   tT_tp_rus = (tau_p_nat - tau_b_nat) / k_M/ TC_tp_rus;   % d, time since birth at puberty at f and T
  tT_tp_norI = (tau_p_nat - tau_b_nat) / k_M/ TC_tsp_norI;   % d, time since birth at puberty at f and T
...
% asp_norI= (tau_p_nat - tau_b_nat) / k_M/ TC_asp_norI + treprod;
 tT_tsp_nor = (tau_p_nat - tau_b_nat) / k_M/ TC_tsp_nor + treprod;
tT_tsp_norI = (tau_p_nat - tau_b_nat) / k_M/ TC_tsp_norI + treprod;

%Length-Weight at first reprod
rT_B_Berg2001 = TC_LWw_Berg2001 * rho_B * k_M;
rT_B_Fleming1996 = TC_LWw_Fleming1996 * rho_B * k_M;
...
% 
  ...
  %test croissance  A exécuter quand la convergence a été faite pour
  %obtenir la courbe de croissance à 7°C et à f=1
