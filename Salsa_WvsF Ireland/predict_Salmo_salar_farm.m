...
s_M  = L_j/ L_b; % -, acceleration factor for f
  [t, LEHR] = ode45(@ode_LEHR, linspace(0, t0, 1000), InitCond,[], par, cPar, f, s_M, temp.tL(:,1), temp.tL(:,2));%from fecondation to start of experiment
%   Eh(1)=0;
%   for i=2:size(LEHR,1)
...
%   
  ...
  %%t-L & t-Ww Ireland
  L_b = l_b * L_m; L_j = l_j * L_m; L_i = l_i * L_m;
  L_0 = V_0^(1/3);  %L0 naissance et non pas début experience (Lb)
