...
TC_tWw = tempcorr(temp.tL, T_ref, T_A);
%   TC_tL = tempcorr(temp.tL, T_ref, T_A);
...
%  Ww_f = L_h^3 * (1 + f * ome); % g, wet weight at hatching at f
  ...
  aT_b = aUL(3,1); % 1 st t where Eh =Ehp  %modified edo solver?: stop when Eh=Ehp, get time at change
...
%   t0 = tL_scoAa(1,1); % time since birth at start of experiment
  ...
  [t, LEHR] = ode45(@ode_LEHR, [0 t0], InitCond,[], par, cPar, f, s_M, temp.tL(:,1), temp.tL(:,2));%from 0 to t0
  
  LEHRf = deval(ode45(@ode_LEHR, data.tL(:,1), LEHR(end,:),[], par, cPar, f, s_M, temp.tL(:,1), temp.tL(:,2)), data.tL(:,1)); %from t0 to tmax
  Lire  = LEHRf(1,:)';
  EWwire = Lire.^3 * (1 + f * ome);
  % output
  ELire = Lire / del_M; % cm, physical length
